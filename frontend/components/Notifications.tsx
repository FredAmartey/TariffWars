  ...
  return <div className="fixed bottom-4 right-4 z-50 flex flex-col space-y-2">
      {notifications.map(notification => <div key={notification.id} className={`${isDarkMode ? 'bg-gray-800 border-gray-700 shadow-lg' : 'bg-white border-gray-200 shadow-md'} border rounded-lg p-4 max-w-md transition-all transform animate-fade-in-up flex items-start`}>
          <div className="flex-shrink-0 mr-3">
            ...
            {notification.type === 'warning' && <AlertCircleIcon className="h-5 w-5 text-yellow-500" />}
          ...
          <button onClick={() => removeNotification(notification.id)} className={`ml-3 ${isDarkMode ? 'text-gray-400 hover:text-gray-300' : 'text-gray-500 hover:text-gray-700'}`}>
