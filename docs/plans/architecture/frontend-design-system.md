...
Three of these deviate from the shadcn defaults deliberately:
...
- **Open every overlay explicitly.** A closed dialog is invisible to both a
  screenshot pass and a DOM crawl, which is exactly where the two most recent
  defects were hiding.
